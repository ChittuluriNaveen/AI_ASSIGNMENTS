...
class State:
    ...
    def evaluate(self):
        ...
        if self.board.is_checkmate():
            # The current player is checkmated, so it's a huge loss for them.
            # The minimax function will choose the move that avoids this.
        ...
        # (a) Material Score - how many pieces each player has
        # A dictionary to hold the values of each piece
        piece_values = {
            chess.PAWN: 1,
            chess.KNIGHT: 3,
            chess.BISHOP: 3.1, # Bishop slightly better than knight
            chess.ROOK: 5,
            chess.QUEEN: 9,
            chess.KING: 0  # King value isn't used in material count
        }
        ...
        # (b) Center Control - having pieces in the center is good
        center_squares = [chess.D4, chess.E4, chess.D5, chess.E5]
        for sq in center_squares:
            piece = self.board.piece_at(sq)
            if piece:
                # Give a small bonus for occupying a center square
                if piece.color == chess.WHITE:
                    score += 0.2
                else:
                    score -= 0.2

        # (c) Mobility - how many moves a player can make
        # We need to check moves for both players, so we copy the board
        temp_board = self.board.copy()
        
        # Check white's moves
        temp_board.turn = chess.WHITE
        white_moves = temp_board.legal_moves.count()

        # Check black's moves
        temp_board.turn = chess.BLACK
        black_moves = temp_board.legal_moves.count()
        
        # A small factor for mobility, so it doesn't overpower material score
        mobility_weight = 0.05
        score += mobility_weight * (white_moves - black_moves)

        # (d) King Safety - it's bad if your king is under attack
        # Find where the white king is
        white_king_square = self.board.king(chess.WHITE)
        if white_king_square:
            # Find how many black pieces are attacking the white king's square
            white_king_attackers = self.board.attackers(chess.BLACK, white_king_square)
            if white_king_attackers:
                # Subtract a penalty for each piece attacking the king
                score -= 0.5 * len(white_king_attackers)

        # Do the same for the black king
        black_king_square = self.board.king(chess.BLACK)
        if black_king_square:
            # Find how many white pieces are attacking the black king's square
            black_king_attackers = self.board.attackers(chess.WHITE, black_king_square)
            if black_king_attackers:
                # Add a bonus for attacking the opponent's king
                score += 0.5 * len(black_king_attackers)
